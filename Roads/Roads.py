...
class Roads():
    ...
    def edge_simulation_july_2022(self, pdAlg1BridgeLevel, BridgeRoadMat, EdgeList, Subnet, Q10, Q11, Q20, Q21, RM,
                                  NumSimRun,
        ...
        for eedge in range(0, len(BridgeRoadMat), 1):
            ...
            for sim_run in range(NumSimRun):
                ...
                if NumBrdg == 1:
                    [TotalAva, TotalCost, pi, time_spent] = self.simulate_cmc(QQ, Simulatingtime, warm_up,
                                                                              RehabFullCost, MaintenanceCost,
                                                                              AvailCoeff)
                elif NumBrdg == 2:
                    [TotalAva, TotalCost, pi, time_spent] = self.simulate_cmcTwoRoads(QQ, QQ2Road, Simulatingtime,
                                                                                      warm_up, RehabFullCost,
                                                                                      MaintenanceCost, AvailCoeff)
                else:
                    [TotalAva, TotalCost, pi, time_spent] = self.simulate_cmcThreeRoads(QQ, QQ2Road, QQ3Road,
    ...
    def simulate_cmc(self, QQ, time, warm_up, RehabFullCost, MaintenanceCost,
                     AvailCoeff):  # ,sim_run,BridgeID,MaintenanceRate,MaintenanceRatePolicy,Edge):
        # time=1100
        # warm_up=500
        TotalCost = 0.0
        # RehabFullCost = 200
    ...
    def simulate_cmcTwoRoads(self, QQ, QQ2Road, time, warm_up, RehabFullCost, MaintenanceCost, AvailCoeff):
        ...
        while clock < time:
            ...
            if clock == 0 or nextUpdate == 1:
                ...
                sojourn_times2Road += [self.sample_from_rate(rate) for rate in
                                       QQL2Road[current_state][current_state + 1:]]
            # Identify the next state
            next_state = min(state_space, key=lambda x: sojourn_times[x])
    ...
    def simulate_cmcThreeRoads(self, QQ, QQ2Road, QQ3Road, time, warm_up, RehabFullCost, MaintenanceCost, AvailCoeff):
        ...
        while clock < time:
            ...
            if clock == 0 or nextUpdate == 2:
                ...
                sojourn_times3Road += [self.sample_from_rate(rate) for rate in
                                       QQL3Road[current_state][current_state + 1:]]
            # Identify the next state
            next_state = min(state_space, key=lambda x: sojourn_times[x])
            next_state2Road = min(state_space2Road, key=lambda x: sojourn_times2Road[x])
            ...
            MinTime = min(clock + sojourn_times[next_state], clock + sojourn_times2Road[next_state2Road],
                          clock + sojourn_times3Road[next_state3Road])
            if MinTime == clock + sojourn_times[next_state]:
                next_stateAll = next_state
                sojourn = sojourn_times[next_state]
                nextUpdate = 0
    ...
    def sampling_inputs_algorithm2(self, resultsPath, pdAlg1BridgeLevel, pdAlg2InputSample, MeasureList, EdgeList,
                                   EdgeNum, Subnet, OverallSample, PlotDist, n_sample):
        for EdgeID in range(0, EdgeNum, 1):
            for sample_run in range(OverallSample):
                ...
                for MeasureId in range(0, 2, 1):
                    ...
                    Sample.append(
                        list(selData[MeasureList[MeasureId]].sample(n=n_sample, replace=True, random_state=1)))
                    Sample.append(1.0)
                    ...
                    if MeasureId == 0:
                        ...
                        lst_dic1.append({'subnet': Subnet, 'sample_run': sample_run,
                                         'Edge': EdgeList[EdgeID], 'Avail': MeasureValue0[0],
                                         'TotalCost': MeasureValue1[0]})
        ...
        if PlotDist == 1:
            for jjj in range(0, 2, 1):
                ...
                ax2.hist(pdAlg2InputSample[Measure], cumulative=1, histtype='step', bins=100, color='tab:orange',
                         density=True)
    ...
    ### Input Alg2 simulation validation
    def inputs_algorithm2_simulation_validation(self, resultsPath, pdAlg1BridgeLevel, pdAlg2InputSample, MeasureList,
                                                EdgeList, EdgeNum, Subnet, OverallSample, PlotDist):
        for EdgeID in range(0, EdgeNum, 1):
            for sample_run in range(OverallSample):
                lst_dic1 = []
                for MeasureId in range(0, 2, 1):
                    selData = pdAlg1BridgeLevel.loc[(pdAlg1BridgeLevel["sim_run"] == sample_run) & (
                                pdAlg1BridgeLevel["Edge"] == EdgeList[EdgeID])]

                    if MeasureId == 0:
                        MeasureValue0 = []
                        MeasureValue0.append(selData[MeasureList[MeasureId]])
                    else:
                        MeasureValue1 = []
                        MeasureValue1.append(selData[MeasureList[MeasureId]])
                        #print(type(MeasureValue0[0]), MeasureValue0[0].apply(lambda x: float(x)))
                        lst_dic1.append({'subnet': Subnet, 'sample_run': sample_run,
                                         'Edge': EdgeList[EdgeID], 'Avail': MeasureValue0[0].apply(lambda x: float(x)),
                                         'TotalCost': MeasureValue1[0].apply(lambda x: float(x))})
